#!/usr/bin/env node
/**
 * sync-commercial-recipes-to-s3.mjs
 *
 * Download Unsplash recipe images and upload to AWS S3
 * Then update the image_url in the database
 *
 * S3 Key format: images/recipes/commercial/{slug}.jpg
 */
...
async function main() {
  ...
  console.log('Commercial Recipe Image Sync to AWS S3');
  console.log('='.repeat(70));
  ...
  try {
    // Get recipes with Unsplash images that need syncing
    ...
    for (const recipe of result.rows) {
      ...
      const s3Key = `images/recipes/commercial/${slug}.jpg`;
    ...
    const verify = await client.query(`
      SELECT COUNT(*) as total,
             COUNT(*) FILTER (WHERE image_url LIKE '%s3.%amazonaws.com%') as on_s3,
             COUNT(*) FILTER (WHERE image_url LIKE '%unsplash%') as on_unsplash
      FROM recipe
    `);
    ...
    console.log(`   Still on Unsplash: ${verify.rows[0].on_unsplash}`);
